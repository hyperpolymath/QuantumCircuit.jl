...
#
# QuantumCircuitMetalExt.jl - Metal GPU kernels for QuantumCircuit.jl
#
# Provides KernelAbstractions-based GPU kernels for quantum circuit simulation
# on Apple Silicon GPUs via Metal.jl. Loaded as a package extension when both
# Metal and KernelAbstractions are available.

module QuantumCircuitMetalExt
...
using AcceleratorGate: MetalBackend

# ============================================================================
# Kernel: Single-Qubit Gate Application
# ============================================================================
#
# Metal uses Float32 natively; we promote to ComplexF64 for quantum precision.
# Each thread processes one pair of amplitudes whose indices differ in the
# target qubit bit position.

@kernel function _metal_apply_single_gate_kernel!(
...
@kernel function _metal_apply_two_qubit_gate_kernel!(
    amps::AbstractVector{ComplexF64},
...
@kernel function _metal_prob_kernel!(
...
@kernel function _metal_diagonal_phase_kernel!(
    amps::AbstractVector{ComplexF64},
...
@kernel function _metal_kronecker_kernel!(
...
"""Return the KernelAbstractions backend for Metal kernel launches."""
_ka_backend() = get_backend(MtlVector{Float32}(undef, 0))
...
function _launch_single_gate!(d_amps, gate_matrix, target, nq)
    ...
    ka = _ka_backend()
    kernel = _metal_apply_single_gate_kernel!(ka, WORKGROUP_SIZE)
...
function _launch_two_qubit_gate!(d_amps, gate_matrix, ctrl, tgt, nq)
    ...
    gate_flat = MtlVector{ComplexF64}(vec(gate_matrix))

    ka = _ka_backend()
    kernel = _metal_apply_two_qubit_gate_kernel!(ka, WORKGROUP_SIZE)
...
function QuantumCircuit.backend_gate_apply(
    ::AcceleratorGate.MetalBackend, amps::Vector{ComplexF64},
    gate_matrix::Matrix{ComplexF64}, target::Int, nq::Int
)
    d_amps = MtlVector{ComplexF64}(amps)
...
function QuantumCircuit.backend_tensor_contract(
    ::AcceleratorGate.MetalBackend, a::Vector{ComplexF64}, b::Vector{ComplexF64}
)
    len_a = length(a)
    len_b = length(b)
    len_c = len_a * len_b

    d_a = MtlVector{ComplexF64}(a)
    d_b = MtlVector{ComplexF64}(b)
    d_c = MtlVector{ComplexF64}(undef, len_c)

    ka = _ka_backend()
    kernel = _metal_kronecker_kernel!(ka, WORKGROUP_SIZE)
...
function QuantumCircuit.backend_measurement(
    ::AcceleratorGate.MetalBackend, amps::Vector{ComplexF64}, nq::Int
)
    dim = length(amps)
    d_amps = MtlVector{ComplexF64}(amps)
    d_probs = MtlVector{Float64}(undef, dim)

    ka = _ka_backend()
    prob_kernel = _metal_prob_kernel!(ka, WORKGROUP_SIZE)
...
function QuantumCircuit.backend_state_evolve(
    ...
    if dim <= 65536  # up to 16 qubits
        ...
        d_coeffs = MtlVector{ComplexF64}(eigenvectors' * amps)
        d_eig_real = MtlVector{Float64}(eigenvalues)
        d_eig_imag = MtlVector{Float64}(zeros(dim))

        ka = _ka_backend()
        phase_kernel = _metal_diagonal_phase_kernel!(ka, WORKGROUP_SIZE)
...
function QuantumCircuit.backend_entangle(::AcceleratorGate.MetalBackend, args...)
    return nothing
end

end # module QuantumCircuitMetalExt
